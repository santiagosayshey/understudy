...
{:else}
	<div class="flex gap-6">
		<img
			src={api.posterImage(page.title.ratingKey, 800)}
			alt=""
			class="bg-surface-raised aspect-2/3 w-40 shrink-0 rounded-lg object-cover"
		/>
					...
					<li>
						{#if c.listed}
							<a
								href="/actors/{c.key}"
								use:link
								class="hover:bg-surface-hover flex flex-col items-center gap-2 rounded-lg p-3 text-center transition-colors"
							>
								{@render member(c)}
							</a>
						{:else}
							<div
								class="flex flex-col items-center gap-2 p-3 text-center opacity-60"
							>
								{@render member(c)}
							</div>
						{/if}
